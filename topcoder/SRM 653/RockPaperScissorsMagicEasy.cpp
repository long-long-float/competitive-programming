...
class RockPaperScissorsMagicEasy {
    ...
    int count(vector<int> card, int score) {
        ull MAX = 1000000007;
        cout << score << " " << card.size() << endl;
        if(score > card.size()) return 0;

        ull comb = 1;
        for(int i = 0;i < score;i++) {
          comb = comb * (card.size() - i) % MAX;
          cout << comb << endl;
        }
        cout << comb << endl;
        for(int i = 1;i <= score;i++) comb /= i;
        
        int n = card.size() - score;
        for(int i = 0;i < n;i++) comb = comb * 2 % MAX;
        return comb;
    }
};
