...
class SingingEasy {
    ...
    int solve(vector<int> pitch) {
        ...
        int len = pitch.size();
        int ans = -1;
        for(int i = pitch.size() - 1;i >= 0;i--) {
          vector<int> pitch3 = pitch;
          vector<int>::iterator endi = remove_if(pitch3.begin(), pitch3.end());
          pitch3.erase(endi, pitch3.end());

          int val = 0;
          for(int j = 1;j < pitch3.size();j++) {
            val += abs(pitch3[j - 1] - pitch3[j]);
          }
          ans = max(ans, val);
        }
